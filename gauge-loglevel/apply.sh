#!/system/bin/sh
# gauge-loglevel: install the module and set the fuel gauge log level live (run as root).
#
...
if [ -w "$GAUGE" ]; then
    echo "[*] writing 0 to FG_daemon_log_level"
    echo 0 > "$GAUGE"
    echo "     now: $(cat "$GAUGE")"
else
    echo "[!] $GAUGE not writable - pwrap gauge device not up yet?"
